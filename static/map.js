    ...
    try {
        ...
        data.nodes.forEach(node => {
            ...
            node.routes?.forEach(route => {
                const prefixLength = route.prefix;
                // Check the oneof fields: either "ipv4" or "ipv6" is set.
                if (route.ipv4 != null && route.ipv4 !== 0) {
                    ...
                    node.routesParsed.push(`${ipStr}/${prefixLength}`);
                } else if (route.ipv6 != null && route.ipv6.length > 0) {
                    // route.ipv6 is a byte array (Uint8Array); convert it to IPv6 notation.
                    const ipv6Bytes = route.ipv6;
                    const groups = [];
                    for (let i = 0; i < ipv6Bytes.length; i += 2) {
                        // Combine two bytes into one group, then convert to hex.
                        const group = ((ipv6Bytes[i] << 8) | ipv6Bytes[i + 1]).toString(16);
                        groups.push(group);
                    }
                    const ipStr = compressIPv6(groups.join(":"));
                    node.routesParsed.push(`${ipStr}/${prefixLength}`);
                } else {
...
function initMap(data) {
    ...
    links.forEach(link => {
        link.source = nodes.find(n => n.asn === link.source) || link.source;
        link.target = nodes.find(n => n.asn === link.target) || link.target;
    });
    ...
    document.getElementById("metadata").innerHTML =
        ...
        `Last update: ${new Date(data.metadata.time).toLocaleString()}`;
...
function showSidebar(node) {
  ...
  `;
    ...
    const updateWithWhois = (whois, asn) => {
        ...
        const prefix = `<a href="${VAR_DN42_EXPLORER_URL}${asn}" target="_blank">View all objects at Collector</a><br/>`;
        const whoisRows = whois
            ...
            .map(l => {
                ...
                if (k.trim() === "remarks") {
                    ...
                    return `<tr><td colspan="2" class="remark">${remark}</td></tr>`;
                }
                return `<tr><td class="key">${k.trim()}</td><td>${vv}</td></tr>`;
            })
            .join("");
        const proceed = prefix + `<div class="whois"><table><tbody>${whoisRows}</tbody></table></div>`;
        const output = header + proceed + neighbors;
    ...
    if (!cache) {
        fetch(VAR_DN42_WHOIS_API, {
            ...
            .catch(error => {
                ...
                sidebarContent.innerHTML = header + "<p>Error querying whois API.</p>" + neighbors;
            });

        // Fallback content while WHOIS info loads
        sidebarContent.innerHTML = header + "<p>Querying whois database...</p>" + neighbors;
    } else {
